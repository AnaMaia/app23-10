...
switch ($acao){
    ...
    case 'alterar':
        ...
        if (!isset($_POST['gravar'])) {
            ...
            $crud = new CrudCliente1();
            $cliente = $crud->getCliente($_GET['idCliente']);

            require __DIR__ . "/../views/perfil_admin/cliente_editar.php";

        }else{


            $novoCliente = new Cliente($_POST['nome_fantasia'], $_POST['cnpj'], $_POST['email'],
                $_POST['razao_social'], $_POST['telefone'], $_POST['endereco'],$_POST['idcli']);

            $crud = new CrudCliente1();
            $crud->editarCliente($novoCliente);

            header('Location: cliente1_controller.php');
        }
        break;

    case 'excluir':        $id_cliente = $_GET['idCliente'];
