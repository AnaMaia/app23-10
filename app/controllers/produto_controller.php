...
function editar ($id) //TA DANDO ERRADO
    {
        ...
        if (isset($_POST['enviar'])){
            $admin1 = new Produto($_POST['nome'],$_POST['preco'],$_POST['referencia'],$_POST['estoque'],$_POST['estoqueMin'],$_POST['descricao'],$_POST['tamanho'],$_POST['cor'],$_POST['tipoProduto'],$_POST['imagem'],$_GET['id']);
            $crud->editar($admin1);
            header("Location: http://localhost/tcc/app/controllers/admin_controller.php?acao=listarAdmin");
        }else{
            $prod = $crud->getProduto($id);
            $tipos = $crud->getTiposProduto();
            $cores = $crud->getCores();
            $tamanhos = $crud->getTamanhos();
            //var_dump($tipos);
            require_once  __DIR__."/../views/perfil_admin/editar_produto.php";
        }
    }
